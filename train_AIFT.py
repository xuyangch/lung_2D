        ...
        for phase in ['aift', 'val', 'test']:
            ...
            if (phase == 'aift'):
                ...
                for i in range(len(dsets['aift'])):
                    ...
                    # get the inputs
                    inputs, labels = dsets['aift'][i]
                    labels = torch.LongTensor(PATCH_NUM * [[labels]])
                    ...
                    if torch.mean(pred) > 0.5:
                        s,_ = torch.sort(pred, dim = 0, descending=True)
                        s = s[0: int(ALPHA * PATCH_NUM)]
                    else:
                        s,_ = torch.sort(pred, dim = 0, descending=False)
                        s = s[0: int(ALPHA * PATCH_NUM)]

                    # Create matrix R
                    for j in range(int(ALPHA * PATCH_NUM)):
                        for l in range(int(ALPHA * PATCH_NUM)):
                            if (j != l):
                                R[i, j, l] = LAMBDA2 * diversity(s, j, l)
                            else:
                                R[i, j, l] = LAMBDA1 * entropy(s, j, l)

                scores = torch.from_numpy(R.sum(axis=(1, 2)))
                _, candidates_need_label = torch.sort(scores, dim = 0, descending=True)
